...
from Constraints import MapConstraints


def test_map_coloring_oz(MRV=False, DH=False, LCV=False, AC3=False):
    # variable and domain initialization
    var = ["WA", "NT", "Q", "NSW", "V", "T", "SA"]
    dom = {i: ["red", "green", "blue"] for i in var}
    ...
    # add constraints to all neighbouring states
    csp.add_constraint(MapConstraints("WA", "SA"))
    csp.add_constraint(MapConstraints("WA", "NT"))
    csp.add_constraint(MapConstraints("NT", "Q"))
    csp.add_constraint(MapConstraints("SA", "Q"))
    csp.add_constraint(MapConstraints("SA", "NSW"))
    csp.add_constraint(MapConstraints("SA", "V"))
    csp.add_constraint(MapConstraints("SA", "NT"))
    csp.add_constraint(MapConstraints("Q", "NSW"))
    csp.add_constraint(MapConstraints("V", "NSW"))
    csp.add_constraint(MapConstraints("V", "T"))

    result = csp.backtrack(LCV=LCV)
...
def test_map_coloring_usa(MRV=False, DH=False, LCV=False, AC3=False):
    us_dict = {
                0: {40, 46, 120, 134, 146, 180, 203},
    ...
    var = [key for key in us_dict.keys()]
    dom = {i: ["red", "green", "blue", "black"] for i in var}
    ...
    for i in us_dict.keys():
        ...
        for val in temp_dict:
            csp.add_constraint(MapConstraints(i, val))
