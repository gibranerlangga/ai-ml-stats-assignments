...
function(job=1)
{
dump("cimcorSP","c:\\QBS124\\cimcorSP.r")
...
for(i in 1:ns)
{
	tabi=read.csv(paste("c:\\QBS124\\stocks\\",symb[i],".csv",sep=""),stringsAsFactors=F)
	ni=nrow(tabi)
...
if(job==1.1) #Save and view a large-format graphics
{
	png("c:\\QBS124\\cimcorSP_1.1.png",height=5000,width=5000)
	par(mfrow=c(ns,ns),mar=c(0,0,0,0),omi=c(.15,.15,.15,.15))
	for(i in seq(from=ns,to=1,by=-1))
	for(j in 1:ns)
	{
