toears <- function(job=1) {
dump("toears","c:\\QBS124\\toears.r")
x=scan("c:\\QBS124\\toears.txt");n=length(x);x10=log10(x)
if(job==1) {
	...
	segments(x10,rep(-1,n),x10,rep(.05,n))
	lines(density(x10),lwd=3)
	segments(x2,-1,x2,.3,col=2);text(x2,.31,paste(round(10^(x2*sd(x10.or)+mean(x10.or)),3)," mg/g",sep=""),col=2)
	}
}
